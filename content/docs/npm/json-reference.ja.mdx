...
| `icon` | string | | カスタムアイコン — URL またはインライン SVG（vendor/type アイコンを上書き） |
...
|-----------|-----|------|------|
| `id` | string | | 一意識別子 |
| `from` | string \| object | ✓ | 接続元（ノードID または `{node, port, ip, plug}` |
...
| `cable` | object | | ケーブル詳細（`medium`, `category`, `length_m`） |
| `style` | object | | カスタムスタイル |
| `metadata` | object | | 任意のメタデータ |
...
### リンク速度（plug.module.standard）

リンク速度は、エンドポイントごとのプラグ内モジュールのイーサネット規格（`plug.module.standard`）として保持されます。速い規格ほど線が太く描画されます。指定例：`1000BASE-T`, `10GBASE-T`, `10GBASE-SR`, `25GBASE-SR`, `40GBASE-SR4`, `100GBASE-SR4`, `100GBASE-LR4`

<Callout type="info">
YAML ではリンクレベルの `standard:` がパーサーによって両端のモジュールにコピーされるショートハンドとして使えますが、JSON はランタイムの `NetworkGraph` 形式としてそのまま読み込まれるため、各エンドポイントの `plug.module.standard` に指定してください。
</Callout>
