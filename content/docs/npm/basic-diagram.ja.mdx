...
`standard:` でリンクのイーサネット規格を指定します。速い規格ほど線が太く描画されます（凡例を有効にすると、使用中の規格ごとの線の太さサンプルが表示されます）。
...
`redundancy` を指定したメンバー群は 1 つの筐体（メガネ形のハル）としてまとめて描画され、
接合面のスタックポートにはラベルがバー内に表示されます。冗長リンク自体は通常のリンクとして
描かれるため、他のリンクと同様にメトリクス（weathermap）を載せられます。3 台以上のスタックは
ハルが横に連結されます。
