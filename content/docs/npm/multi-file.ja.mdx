...
<Callout type="warn">
CLI（`npx @shumoku/cli render`）は現在、入力を単一ファイルとしてパースし、`file:` 参照を解決**しません**。マルチファイル解決は `HierarchicalParser` を使ったプログラムからの利用、および Shumoku サーバーで利用できます。
</Callout>
